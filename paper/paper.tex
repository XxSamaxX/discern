...
Past $17.33$ nats the probability is no longer representable in float32,
the dtype most inference pipelines carry, and on MMBench that covers 62\% of
all items, where every probability is exactly 1.0; within that collapsed set
the gap still separates correct from incorrect at AUC 0.736. Thresholding the
...
\item A correction to a claim that is easy to make and that we made
      ...
      which the probability collapses to exactly 1.0 on 62\% of items while the
      gap still ranks them at AUC 0.736. Thresholding the gap supports
      abstention at 98.4\% accuracy over 66\% of items
...
$-\ln 2^{-25} = 17.33$ nats the value is no longer representable in float32, the dtype most inference pipelines carry. On MMBench
that is \textbf{62\% of all items}, where every probability is exactly 1.0 and
the ranking is gone; in float16, whose collapse threshold is only 8.32 nats, it is 81\%. Within that collapsed set the gap
still separates correct from incorrect answers at AUC 0.736, and in the float16
set at 0.867. That is information the readout computed and the representation
threw away.
...
threshold must be set per question shape rather than once. Use semantic ifs for
