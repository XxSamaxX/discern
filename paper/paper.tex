...
\author{%
Samuel Otero Agraso\\
\small\url{https://github.com/XxSamaxX/semif-vision}
}
\date{}
...
unchanged to a vision--language model and evaluate it on a small hand-built
benchmark of 22 perceptual decisions over four photographs. Qwen3-VL-4B answers
all 22 correctly at 110--140\,ms per decision on a single consumer GPU,
including families that are adversarial for such models: counting, absence,
a false-premise question, an unanswerable question, and a brand mark spanning
roughly $20\times15$ pixels. Rotating the answer options never changes the
winner. We then show that the probability these systems report is not usable:
the softmax over answer slots assigns at least $0.9993$ to the winning option
in \emph{every one} of the 22 decisions, while the underlying logit gap ranges
from 7.3 to 26.4 nats and orders the decisions by genuine difficulty. Position
bias, measured directly, is only 1--2 nats, which explains why order
calibration changes nothing above a gap of roughly 3 nats. Finally we test
whether the gap can serve as a safety gate that detects questions the model
should not answer. It cannot. On 12 normative questions---policy judgements
no model can ground---the gap distribution shifts significantly
(median 8.8 vs 20.9 nats, AUC 0.788, $p=0.006$) but overlaps so heavily that
the best possible threshold still admits 7 of the 12. The model asserts that
a photograph requires legal consent at 15.6 nats, as confidently as it reports
that no dog is present at 26.4. We conclude that the logit gap is a usable
difficulty signal \emph{within} a question family and not an epistemic one
across families.
\end{abstract}
...
\item An accuracy result on a small adversarial benchmark: 22/22, with zero
      order flips (Section~\ref{sec:accuracy}).
\item A negative result about the reported probability. The softmax over
      answer slots saturates and destroys the confidence signal; the raw logit
      gap preserves it (Section~\ref{sec:saturation}).
\item A direct measurement of position bias at 1--2 nats, which quantifies when
      order calibration is and is not needed (Section~\ref{sec:order}).
\item A second negative result: the gap does not separate answerable perceptual
      questions from ungroundable normative ones well enough to gate them
      (Section~\ref{sec:families}). This is the finding we consider most
      consequential for anyone deploying the pattern.
\end{enumerate}
...
item it found hardest from the item it found easiest. The information is not
...
\paragraph{Limitations.} The benchmark is small: 4 images, 22 perceptual and
12 normative decisions, one model, one prompt template, one language. Ground
truth is author-written and single-annotator, and one label was demonstrably
wrong before correction. The normative set is not perfectly controlled, as
noted. The 22/22 result means our items failed to locate this model's
decision boundary, so it bounds nothing; a benchmark that separates models
would need items the model actually fails. All measurements come from a single
machine and a single run, without repetition or confidence intervals on
latency.
...
\url{https://github.com/XxSamaxX/semif-vision}. The scoring engine is vendored
